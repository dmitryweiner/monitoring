...
Дата: 2026-09-14. Разработка приостановлена по решению владельца; следующие
изменения выполняются непосредственно на Orange Pi 4 Pro. Документ предназначен
...
- Облачный backend: TypeScript, Wrangler 4.129.1, Miniflare 5.20260907.0-alpha.
...
- Установлен ARM64 Node 24.21.0 (проверено 14 сентября). Wrangler/Miniflare требуют
  ...
  ARM64 workerd на этой плате ещё не запускался; его совместимость нужно подтвердить.
...
R2 и workers.dev на последней проверке требовали активации владельцем аккаунта.
Worker, bucket, ключи приложения и доставка ещё не настроены.

Wrangler был авторизован только на исходном компьютере; на Orange Pi выполнить
собственный вход при продолжении работ. Не помещать OAuth, GitHub-токены,
пароль SSH, device.token, admin.key или VPN-конфигурацию в Git.
Секреты приложения ещё не генерировались. Генератор monitoring.admin пока создаёт
server.env с именами прежнего backend; соответствие именам Worker описано в README.
...
1. Проверить актуальное состояние платы; установить зависимости проекта под имеющийся Node 24.
2. Прогнать Miniflare, устранить несовместимости; проверить OpenAPI и SQL-квоты.
3. Завершить Cloudflare: закрытый R2, ключи, Worker, реальные тесты API.
4. Настроить URL/token агента, убрать только collect-only.conf, выполнить
   daemon-reload и restart агента; проверить доставку и догрузку без дубликатов.
5. Авторизовать Tailscale, проверить обычный SSH. Изучить существующий mihomo,
   проверить/адаптировать обход TUN, DNS и DERP; изменения проводить с откатом.
6. Реализовать экспорт/восстановление D1/R2; выполнить приёмку и 48 часов наблюдения.
7. Подключить и испытать BMP280/DHT11, когда оборудование готово.
