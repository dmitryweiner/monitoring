...
| Orange Pi: /etc/monitoring/agent.toml | Рабочая конфигурация с адресом Worker; рядом agent.toml.bak — копия до правки |
...
и с тех пор работает. Tailscale показывает NeedsLogin.
...
Не считать выполненными удалённый SSH при блокировке VPN, восстановление
резервной копии и 48-часовую приёмку — доказательств пока нет.
